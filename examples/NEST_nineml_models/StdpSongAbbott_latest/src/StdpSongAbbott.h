...
public:
  ...
  check_connection( Node& s, Node& t, rport receptor_type, const CommonPropertiesType& )
  {
    ...
    t.register_stdp_connection( tlast_pre - get_delay(), get_delay() );
  }
...
STDPConnection_StdpSongAbbott< targetidentifierT >::STDPConnection_StdpSongAbbott( const STDPConnection_StdpSongAbbott< targetidentifierT >& rhs )
  ...
  , wmax( rhs.wmax )
  , wmin( rhs.wmin )
  , tlast_post( rhs.tlast_post )
  , tlast_pre( rhs.tlast_pre )
  , deltaw( rhs.deltaw )
  , M( rhs.M )
  , P( rhs.P )
  , wsyn( rhs.wsyn )
...
STDPConnection_StdpSongAbbott< targetidentifierT >::get_status( DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::A_LTP, aLTP);
  def< double >( d, names::tau_minus, tauLTD);
  def< double >( d, names::A_LTD, aLTD);
...
STDPConnection_StdpSongAbbott< targetidentifierT >::set_status( const DictionaryDatum& d, ConnectorModel& cm )
{
  ...
  updateValue< double >( d, names::A_LTP, aLTP);
  updateValue< double >( d, names::tau_minus, tauLTD);
  updateValue< double >( d, names::A_LTD, aLTD);
...
STDPConnection_StdpSongAbbott< targetidentifierT >::send( Event& e, thread t, const CommonSynapseProperties& )
{
  ...
  // details.
  target->get_history( tlast_pre - dendritic_delay, t_spike - dendritic_delay, &start, &finish );

  // facilitation due to post-synaptic spikes since last pre-synaptic spike
  double minus_dt;
  while ( start != finish )
  {
   minus_dt = - ( start->t_ - (tlast_pre - dendritic_delay) );
  ...
  P = P * std::exp( (tlast_pre - t_spike) / tauLTP ) + aLTP;
