...
def test_record_native_model():
    ...
    assert data[0].name == 'apical.v'
    assert data[1].name == 'soma.ina'
    assert data[0].sampling_rate == 10.0 * pq.kHz
    assert data[0].units == pq.mV
    assert data[1].units == pq.mA / pq.cm**2
    assert data[0].t_start == 0.0 * pq.ms
    assert data[0].t_stop == 250.1 * pq.ms  # would prefer if it were 250.0, but this is a fundamental Neo issue
    assert data[0].shape == (2501, 10)
    return data
